...
def minimax(board):
    ...
    new_board = copy.deepcopy(board)
    if player(board) == X:
        # Init max_value with better than possible alpha and beta
        v, best_action = max_value(board, -2, 2)
    else:
        # Init min_value with better than possible alpha and beta
        v, best_action = min_value(board, -2, 2)
    return best_action

# Using alpha-beta pruning
# alpha is best max so far, beta is best min so far
def max_value(state, alpha, beta):
    ...
    for action in actions(state):
        temp_alpha, _ = min_value(result(state, action), alpha, beta)
        if temp_alpha > alpha:
            alpha = temp_alpha
            best_action = action
        if alpha >= beta:
...
def min_value(state, alpha, beta):
    ...
    best_action = None
    for action in actions(state):
        temp_beta, _ = max_value(result(state, action), alpha, beta)
        if temp_beta < beta:
            beta = temp_beta
            best_action = action
        if beta <= alpha:
